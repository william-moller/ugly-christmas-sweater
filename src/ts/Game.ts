...
export class Game {
    ...
    private onFinishAssign: (() => void) | null = null;
    ...
    */
    ...
    private setupHandStock() {
        ...
        this.cardsManager = new BgaCards.Manager({
            ...
            isCardVisible: () => true,
            setupDiv: (_c: SweaterCard, div: HTMLElement) => {
    ...
    private bankedFadVpForBuild(playerId: number, buildNo: number): number {
        const claims = this.expressClaims();
        let total = 0;
        for (const fadId of Object.keys(claims)) {
            const c = claims[Number(fadId)];
    ...
    private buildPublicScore(cards: SweaterCard[], playerId: number, buildNo: number): number {
        ...
        if (!bySlot.L || !bySlot.R || !bySlot.B) return banked; // not a completed L+R+B sweater
        ...
        for (const f of fads) {
            if (f.clash) {
                if (allDiffColor && allDiffIcon) derivedFad += VP.fad;
                continue;
            }
            (f.objectives ?? []).forEach((o: any) => {
                if (o.match === 'color' && allSameColor && colors[0] === o.value) { derivedFad += VP.fad; colorIsFad = true; }
                if (o.match === 'icon' && allSameIcon && icons[0] === o.value) { derivedFad += VP.fad; iconIsFad = true; }
            });
        }
    ...
    public beginAssignPatches(
        cardIds: number[],
        onAssign: (cardId: number, value: number, icon: string) => void,
        onFinish?: () => void,
    ) {
    ...
    private updateAssignTitle() {
        ...
        if (done) {
            ...
            sb.addActionButton(_('Skip — set it at the end of the round'),
                () => { this.endAssignPatches(); done(); }, { color: 'secondary' });
            return;
        }
