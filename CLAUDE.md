...
**Session 2026-07-02 (client UI tweaks) — bigger hand, compact rotated Secret Santa, centred shared-zone stack. Client-only (`src/ts/Game.ts` + `src/scss/Game.scss`); builds clean (`npm run build`, exit 0); NOT yet SFTP-synced or table-tested.** Three visual reworks, no server/data change:

- **Enlarged the player's hand (desktop).** The fanned `bga-cards` `HandStock` frame + fan geometry come from the manager's `cardWidth`/`cardHeight` — bumped 64/90 → **96/135** (~1.5×) in `setupHandStock`, with `cardOverlap` 60 → **90** (kept proportional so the fan spread reads the same). Because the shared `cardFaceInner` markup sizes its contents off `--ucs-card-w` (not the manager px), `#ucs-my-hand-wrap` now sets `--ucs-card-w: 96px`/`--ucs-card-h: 135px` so the face content scales *with* the frame; `#ucs-my-hand` min-height 124 → 186px. The manager fixes the frame px in JS (no CSS var to media-query), so the `@media (max-width:800px)` block scales the whole fan back with `transform: scale(0.68)` (origin top center) to keep the mobile footprint ~original.
- **Secret Santa = a normal card turned 90°, tucked beside the Round Parameters** (was its own full-width band wasting the horizontal space). `renderSecretSanta` wraps each card in a new `.ucs-santa-slot` whose width/height are the card's height/width **swapped**, reserving the rotated (landscape) footprint so the `transform: rotate(90deg)` card doesn't overflow/overlap (Express deals 2). Placeholder name text rotates with the card (sideways) — fine for now; counter-rotate the label if upright text is wanted once art lands.
- **Re-centred the shared zones into a vertical stack, opponents raised to the top-right.** New `#ucs-upper` flex row: `[.ucs-upper-spacer 210px] [#ucs-center-stack flex:1] [#ucs-opponents 210px]`, `align-items:flex-start` (opponents ride up level with the gameplay cards). The left spacer mirrors the opponents' width so `#ucs-center-stack` (`align-items:center`) is balanced about the true page centre. Stack order top→bottom: **Gameplay cards → Draft Pool → Trade Area**, each centred; the Trade Area gets a **3px solid black border** (`#ucs-trade-area`) as the current-trick frame; draft/trade `.ucs-card-row`s use `justify-content:center`. `#ucs-params-row` became a `1fr · auto · 1fr` grid so the gameplay cards stay page-centred with the Secret Santa in the right track (doesn't shove them off-centre). Removed the old `#ucs-main` / `#ucs-shared-row` containers; `#ucs-my-area` now spans full width below the stack; hand stays at the bottom. Responsive block updated (`#ucs-upper` stacks, spacer hidden, opponents → chips as before).
- **Verify on Studio:** hand renders larger and still fans/selects/animates (and scales down on a narrow viewport); Secret Santa sits as a small turned card beside the Round Parameters without overlap (Casual 1 / Express 2); the centred Gameplay→Draft→Trade stack with the black-framed Trade Area; opponents raised to the top-right; mid-width (~800–1100px) doesn't crowd the stack against the opponents (if it does, cap the stack width or trim the spacer).
