...
- **TEMP orientation placeholder (`Game::placeDraftedCard`):** orientation is printed on each card (`Material::FACES`, pending art), so as a stopgap drafted cards now **cycle L → R → B**, filling the current build and starting a new one once it completes. This is throwaway — it only exists so the build / round-end / scoring loop is testable before the art. Replace wholesale with printed-orientation + player-chosen placement (search `placeDraftedCard` for `TODO(art)`). Until then, every 3 drafts auto-complete a sweater, so the round-end trigger (3rd sweater) and `ScoreRound` can be exercised now.

Next steps (engine + board UI proven; remaining work):
1. **Test the round-end → scoring → next-round loop on BGA** now that the temp orientation placeholder lets sweaters complete (every 3 drafts). Confirm the 3rd-sweater trigger, `ScoreRound` (base +2/sweater so far), and `NewRound` all run through the UI. *(Trade→Draft→cleanup loop already verified on BGA 2026-06-19.)*
...
3. Complete rule internals: Perfect Fit / Trendy Yarn / Ultimate-Trump resolution, full scoring (runs need only values; Fad/non-fad/Secret Santa need icons), patch wilds, and **real placement** — replace the temp L→R→B cycling in `placeDraftedCard` with printed orientation + player-chosen build / "place over".
