...
class TinaTink {
    ...
    onEnteringState(_args, isCurrentPlayerActive) {
        if (!isCurrentPlayerActive || !this.game.myUnusedBonus('tina')) {
            return;
        }
...
class Game {
    ...
    */
    ...
    renderKnitting(playerId, targetEl) {
        ...
        Object.keys(builds)
            ...
            builds[buildNo].forEach((card) => {
                ...
                if (Number(card.id) === this.assignFocusId)
                    el.classList.add('ucs-assigning');
                this.attachTooltip(el, card);
    ...
    renderScorePlayer(p) {
        ...
        el.insertAdjacentHTML('beforeend', `<div class="ucs-score-pname"><span class="ucs-score-pn">${p.player_name}</span>`
        ...
        if (!p.sweaters.length)
            sweaters.innerHTML = `<div class="ucs-empty">${_('No sweaters')}</div>`;
        ...
        (p.secretSantas || []).forEach((ss) => {
            ...
            el.insertAdjacentHTML('beforeend', `<div class="ucs-score-santa ${ss.satisfied ? 'ucs-santa-yes' : 'ucs-santa-no'}">`
    ...
    renderScoreSweater(s) {
        ...
        (s.cards || []).forEach((card) => {
            ...
            if (slot) {
                el.style.gridArea = slot;
                el.classList.add(`ucs-slot-${slot}`);
                filled.add(slot);
            }
            else
                el.classList.add('ucs-floating');
            build.appendChild(el);
        });
        if (filled.size > 0) {
            ['L', 'R', 'B'].forEach((sl) => { if (!filled.has(sl))
                build.appendChild(this.makeEmptySlot(sl)); });
        }
        ...
        if (!s.complete) {
            ...
            if (s.parts.run)
                li(`${_('Consecutive values')} +${s.parts.run}`);
            if (s.parts.fad)
                li(`${_('Fad')} +${s.parts.fad}`);
            if (s.parts.nonfad)
                li(`${_('All one colour/icon')} +${s.parts.nonfad}`);
        }
