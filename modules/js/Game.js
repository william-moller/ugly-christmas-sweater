...
class AssignPatches {
    ...
    onEnteringState(args) {
        ...
        this.game.beginAssignPatches(mine.map(Number), (cardId, value, icon) => {
...
class BillyChoice {
    ...
    onEnteringState(_args, isCurrentPlayerActive) {
        ...
        this.game.beginBillyChoice(() => this.bga.actions.performAction('actBillyActivate', {}), () => this.bga.actions.performAction('actBillySkip', {}));
    }
    onLeavingState() {
...
class TinaTink {
    ...
    onEnteringState(args) {
        ...
        this.game.beginTinaTink((cardId, buildNo, slot) => this.bga.actions.performAction('actTinaMove', { card_id: cardId, build_no: buildNo, slot }), (cardA, cardB) => this.bga.actions.performAction('actTinaSwap', { card_a: cardA, card_b: cardB }), () => this.bga.actions.performAction('actTinaSkip', {}));
    }
    onLeavingState() {
 ...
 */
function secretSantaTooltip(ss, vp, owner, done) {
...
class Game {
    constructor(bga) {
        ...
        this.onTinaMove = null;
        this.onTinaSwap = null;
        this.onTinaSkip = null;
        this.tinaMode = null;
        this.tinaSelA = null;
        this.tinaSelB = null;
        this.tinaBuildNo = null;
        this.tinaSlot = null;
    ...
    */
    ...
    santaHeadEl(ss, domId, owner) {
        ...
        wrap.appendChild(el);
        if (ss.done !== undefined)
            wrap.appendChild(this.santaVerdict(ss.done));
        return wrap;
    }
    ...
    renderSecretSanta() {
        ...
        cards.forEach((c) => {
            ...
            if (done.has(arg))
                slot.appendChild(this.santaDoneTick());
    ...
    renderRoundTracker(express) {
        const col = document.getElementById('ucs-rt-col');
        if (!col)
            return;
        col.innerHTML = '';
        if (express)
            col.appendChild(this.roundTrackerEl(express));
    }
    ...
    layoutTallStrip() {
        ...
        if (!table || !upper || !strip || !centre || !lower)
            return;
        ...
        if (want === (lower.parentElement === upper))
            return;
        if (want)
    ...
    wideLayoutFloor() {
        ...
        if (this.gamedatas.express) {
            if (count <= 2) {
                fixed = 916;
            } // 3 Fads → a five-card parameter row
            else if (count === 3) {
                fixed = 764;
            }
            else {
    ...
    narrowMq() {
        return (this.narrowMqCache ?? (this.narrowMqCache = window.matchMedia(`(max-width: ${this.wideLayoutFloor() - 1}px)`)));
    }
    /**
    ...
    layoutNarrowSidebar() {
        ...
        if (active) {
            ...
            if (lift && lift.parentElement !== upper)
                upper.appendChild(lift);
            ...
            const strip = document.getElementById('ucs-board-strip');
            if (right && oppo.parentElement !== right)
                right.appendChild(oppo);
            ...
            if (santa && strip && santa.parentElement !== strip)
                strip.appendChild(santa);
            if (santaOne && strip && santaOne.parentElement !== strip)
                strip.insertBefore(santaOne, santa);
            if (this.rtTopRight()) {
                if (right && rt.parentElement !== right)
                    right.insertBefore(rt, oppo);
            }
    ...
    roundTrackerEl(express) {
        ...
        for (let n = 1; n <= 12; n++) {
            ...
            if (n === current)
                cell.classList.add('ucs-rt-now');
            else if (n < current)
                cell.classList.add('ucs-rt-past');
            if (n % rotateEvery === 0) {
    ...
    gameplayCardEl(type, card, domId) {
        const el = document.createElement('div');
     ...
     */
    flipCardFrom(el, from, durationSec) {
        if (!el || !from || !this.bga.gameui.bgaAnimationsActive?.())
            return Promise.resolve();
        const table = document.getElementById('ucs-table');
        const tScale = (table && table.offsetWidth) ? table.getBoundingClientRect().width / table.offsetWidth : 1;
        const now = el.getBoundingClientRect();
        if (!now.width || !from.width)
            return Promise.resolve();
     ...
     */
    revealFlip(el, durationSec = 0.5) {
        if (!el || !this.bga.gameui.bgaAnimationsActive?.())
            return Promise.resolve();
    ...
    gpActiveIds() {
        const gp = this.gamedatas.gameplay;
        const out = {};
        ['perfectfit', 'trendyyarn', 'fad'].forEach((t) => {
     ...
     */
    async revealChangedParameters(before) {
        const after = this.gpActiveIds();
        const changed = Object.keys(after).filter((t) => after[t] && before[t] !== after[t]);
        if (!changed.length)
            return;
        await Promise.all(changed.map((t) => this.revealFlip(document.getElementById(`ucs-gp-face-${t}`))));
    }
    /**
     ...
     */
    fadeCardOut(el, durationSec = 0.4) {
        if (!el || !this.bga.gameui.bgaAnimationsActive?.())
            return Promise.resolve();
    ...
    claimedFadsForBuild(playerId, buildNo) {
    ...
    renderKnittingCompact(zone, playerId, cards) {
        ...
        Object.keys(builds).map(Number).sort((a, b) => a - b).forEach((buildNo) => {
            ...
            if (takenSlots.size > 0) {
                SLOTS.forEach((s) => {
                    if (!takenSlots.has(s))
                        group.appendChild(this.makeMiniEmptySlot(s));
                });
            }
    ...
    renderHand(dealFrom) {
        ...
        this.handSync = this.handSync
            ...
            if (hand.length) {
                await this.handStock.addCards(hand, dealFrom ? { fromElement: dealFrom } : undefined, dealFrom ? 80 : undefined);
            }
    ...
    fanLift(cards, currentLift) {
        ...
        cards.forEach((el) => {
            ...
            if (!b.width)
                return; // hidden / mid-teardown: contributes no geometry
            if (b.left >= leftEdge && b.right <= rightEdge)
                return; // clear of both corners
            lowest = Math.max(lowest, b.top + b.height * 0.55);
        });
        if (!isFinite(lowest))
            return 0; // nothing reaches a corner — sit where the library put us
    ...
    clearPatchCopy() {
        this.selectedPlayId = null;
        if (this.patchCopyPatchId == null && this.patchCopySourceId == null)
            return;
        this.patchCopyPatchId = null;
        this.patchCopySourceId = null;
        this.renderDraftPool();
    ...
    renderPlacementPanel() {
        ...
        if (!patch) {
            ...
            if (this.confirmMode() === 0 && !this.myUnusedBonus('maria')) {
    ...
    renderMariaPanel(card) {
        const sb = this.bga.statusBar;
        sb.removeActionButtons();
        this.renderKnitting(this.myId);
        sb.setTitle(_('Mixed-up Maria: choose a sweater and any orientation'));
        const { buildNos } = this.myBuilds();
        sb.addActionButton(_('+ New sweater'), () => { this.mariaBuildNo = 0; this.renderPlacementPanel(); }, { color: this.mariaBuildNo === 0 ? 'primary' : 'secondary' });
        buildNos.forEach((b) => sb.addActionButton(`${_('Sweater')} ${b}`, () => { this.mariaBuildNo = b; this.renderPlacementPanel(); }, { color: this.mariaBuildNo === b ? 'primary' : 'secondary' }));
        SLOTS.forEach((s) => sb.addActionButton(orientationName(s), () => { this.mariaSlot = s; this.renderPlacementPanel(); }, { color: this.mariaSlot === s ? 'primary' : 'secondary' }));
    ...
    submitMariaDraft() {
        if (this.selectedDraftId == null || !this.onDraftComplete || this.mariaBuildNo == null || !this.mariaSlot)
            return;
        const placement = {
    ...
    beginBillyChoice(onActivate, onPass) {
        const sb = this.bga.statusBar;
        sb.removeActionButtons();
    ...
    beginTinaTink(onMove, onSwap, onSkip) {
    ...
    renderTinaPanel() {
        ...
        if (this.tinaMode === 'move') {
            ...
            if (this.tinaSelA == null)
                sb.setTitle(_('Click the first piece to swap'));
            else if (this.tinaSelB == null)
                sb.setTitle(_('Click the second piece to swap'));
            else {
     ...
     */
    showRoundReview(detail, isCurrentPlayerActive, onContinue) {
        if (!isCurrentPlayerActive) {
    ...
    hideRoundSummary() {
        ...
        if (this.sheetAnimTimer !== null) {
            clearTimeout(this.sheetAnimTimer);
            this.sheetAnimTimer = null;
        }
    ...
    showRoundSummaryChip() {
        if (document.getElementById('ucs-score-restore'))
            return;
     ...
     */
    renderRoundSummary(detail, onOkay, modal = true) {
        this.hideRoundSummary();
        const overlay = document.createElement('div');
    ...
    buildScorepadTable(detail) {
        ...
        const cellOf = (pid, r) => rounds.find((x) => x.round === r)?.players[pid];
        ...
        if (detail.bonus)
            cats.push({ key: 'bonus', label: _('Bonus'), vp: '+3 VP' });
        const sumCat = (pid, key) => rounds.reduce((acc, x) => acc + (Number(x.players[pid]?.[key]) || 0), 0);
        ...
        html += '</tr></thead><tbody>';
        // ---- scoring rows ----
        const cell = (v, extra = '') => `<td class="ucs-sp-num ${extra}">${v === undefined ? '' : v}</td>`;
        ...
        const santaCellMounts = [];
        const santaCellHtml = (p, r, rec) => {
    ...
    async notif_santaProgress(args) {
    ...
    async notif_trickCleanup(args) {
        ...
        [...this.cardArray(this.gamedatas.trick), ...this.cardArray(this.gamedatas.draftpool)].forEach((c) => {
            const el = document.getElementById(`ucs-card-${c.id}`);
            if (el)
                oldRects[Number(c.id)] = el.getBoundingClientRect();
        });
     ...
     */
    flipFromRects(oldRects, durationSec) {
        ...
        Object.keys(oldRects).forEach((key) => {
            ...
            if (!el)
                return; // card isn't on the table after the re-render (covered / discarded) — skip
            const now = el.getBoundingClientRect();
    ...
    knittingRects(playerId) {
        ...
        Object.values(this.gamedatas.knitting)
            ...
            if (el)
                rects[Number(c.id)] = el.getBoundingClientRect();
        });
        return rects;
    }
    /**
    ...
    async notif_newRoundPrivate(args) {
        ...
        this.renderHand(document.querySelector('#ucs-my-pile .ucs-pile-card'));
        this.renderSecretSanta();
    }
